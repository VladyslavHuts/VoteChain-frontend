    ...
    try {
      ...
      if (data.success && data.vote) {
        this.setState({
          ...
          transactionAddress: data.vote.transactionAddress, // Додаємо transactionAddress в стан
        });
      }
    } catch (error) {
  ...
  fetchVotingData = (id: string) => {
    fetch(`http://localhost:80/votes/${id}/details`)
      ...
      .then((data) => {
        ...
        // Перевірка, чи дата завершення більше поточної
        const isVotingClosed = new Date(voting.endTime) < new Date();
        ...
        this.setState({
          ...
          contractAddress: voting.contractAddress, // Зберігаємо адресу контракту
          isClosed: isVotingClosed, // Встановлюємо true, якщо голосування завершилось
        });
      })
      .catch((error) => {
  ...
  handleVote = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:80/votes/${id}/vote/${selectedOption}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${authToken}`,
          ...
          <div className="voting__window">
            ...
            <div className="voting__info">
              <div className="voting__info">
                <p className="voting__address">
                  Vote Address:{" "}
                  <span id="address">
                    {this.state.contractAddress || "Loading..."}
                  </span>
                </p>
                <SlCopyButton value={this.state.contractAddress || ""}>
                  Copy
                </SlCopyButton>
              </div>
            </div>
            <div className="voting__info">
              {this.state.transactionAddress &&
                this.state.transactionAddress !== "Loading..." && (
                  <div className="voting__info">
                    <p className="voting__address">
                      Option Address:{" "}
                      <span id="address">{this.state.transactionAddress}</span>
                    </p>
                    <SlCopyButton value={this.state.transactionAddress}>
                      Copy
                    </SlCopyButton>
                  </div>
                )}
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="voting__bar-percentage-container">
                          <div className="voting__bar-percentage">
                            {actualPercentage}%
                          </div>
                        </div>
